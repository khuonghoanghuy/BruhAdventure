...
class PlayState8 extends MainState
{
	var onLevel:Int = 0;
	...
	override public function create()
	{
		...
		map = new FlxOgmo3Loader(AssetPaths.level__ogmo, AssetPaths.level8__json);
	...
	function win_yeah(player:Player, flag:Flags)
	{
		if (player.alive && player.exists && flag.alive && flag.exists)
		{
			flag.kill();
			#if desktop
			sys.io.File.saveContent("assets/data/data_score/lv8.txt", Std.string(Stuff.SCORE));
			#end
			Stuff.SCORE = 0;
			Stuff.WASHIT = 0;
